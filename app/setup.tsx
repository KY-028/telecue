...
const Handle = ({ side, margin, onDragEnd, windowWidth }: { side: 'left' | 'right', margin: SharedValue<number>, onDragEnd: (val: number) => void, windowWidth: number }) => {
    const context = useSharedValue({ startMargin: 0 });

    const maxMargin = Platform.OS === 'web' ? (windowWidth - 300) / 2 : 120;
    ...
    const pan = Gesture.Pan()
        ...
        .onUpdate((e) => {
            ...
            // Clamp margin
            margin.value = Math.max(0, Math.min(newMargin, maxMargin));
        })
        .onEnd(() => {
    ...
    return (
        <GestureDetector gesture={pan}>
            <Animated.View
                style={[
                    { position: 'absolute', top: 0, bottom: 0, width: 30, alignItems: 'center', justifyContent: 'center', zIndex: 50, ...Platform.select({ web: { cursor: 'col-resize' } }) } as any,
                    animatedStyle
                ]}
            >
...
const DraggableMarginPreview = ({ activeScript, updateSettings }: { activeScript: any, updateSettings: any }) => {
    ...
    // Default Web Margin Logic:
    // If on web and margin appears unset (0) or too small for a desktop layout (< 200),
    // default to centered 800px width.
    const margin = useSharedValue(activeScript.margin);

    // Sync shared value with store updates (e.g. from other sources or reloads),
    // but we need to be careful not to override the drag gesture.
    // However, since handle drives shared value directly, and onEnd updates store,
    // this sync mostly handles the "initial render" or "external update" case.
    useEffect(() => {
        // Clamp visual margin on mobile to avoid breaking layout if a large "Web" margin is saved
        if (Platform.OS !== 'web' && activeScript.margin > 120) {
            margin.value = 120;
            return;
        }
        margin.value = activeScript.margin || 0;
    }, [activeScript.margin]);

    // One-time initialization for Web Desktop Defaults
    useEffect(() => {
        // Only run this logic once per script load to avoid fighting with user adjustments.
        const isMobileMargin = !activeScript.margin || activeScript.margin < 200;

        if (Platform.OS === 'web' && isMobileMargin && windowWidth > 800) {
            const defaultWebMargin = Math.max(0, (windowWidth - 800) / 2);

            // Apply default
            margin.value = defaultWebMargin;
            runOnJS(updateSettings)({ margin: Math.round(defaultWebMargin) });
        }
    }, [activeScript.id]); // Run only when script changes, NOT when margin changes.
    ...
    const contentStyle = useAnimatedStyle(() => {
        return {
            paddingLeft: margin.value,
            paddingRight: margin.value,
        };
    });
    ...
    return (
        <View className="h-64 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 mb-8 relative rounded-xl overflow-hidden">
            <Text className="text-zinc-400 dark:text-zinc-500 text-xs absolute top-2 left-0 right-0 text-center uppercase tracking-widest z-10 pointer-events-none select-none">{i18n.t('previewDrag')}</Text>

            <Animated.View
                className="flex-1 bg-black"
                style={[contentStyle, mirrorTransform, { width: '100%', height: '100%', backgroundColor: 'black', overflow: 'hidden' }]}
            >
                <ScrollView
                    scrollEnabled={false}
                    showsVerticalScrollIndicator={false}
                    contentContainerStyle={{ flexGrow: 1, alignItems: 'center', paddingVertical: 40 }}
                >
                    <Text
            ...
            <Handle side="left" margin={margin} onDragEnd={updateStore} windowWidth={windowWidth} />
            <Handle side="right" margin={margin} onDragEnd={updateStore} windowWidth={windowWidth} />
        </View>
    );
};
                        ...
                        <View className="flex-row gap-4 mb-8">
                            <TouchableOpacity
                                ...
                                onPress={(e) => {
                                    ...
                                    updateActiveScriptSettings({ mode: 'phone' });
                                }}
                            >
                            ...
                            <TouchableOpacity
                                ...
                                onPress={(e) => {
                                    ...
                                    updateActiveScriptSettings({ mode: 'rig' });
                                }}
                            >
